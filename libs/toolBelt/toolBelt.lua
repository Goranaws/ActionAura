...
function toolbelt.Slider(parent, optionDetails)
	...
	slider.high = slider:CreateLine(nil, 'OVERLAY', 2); do
		...
		slider:SetScript("OnEnter", function() if slider:IsEnabled() == true then slider.high:Show() end end)
	...
	contain.OnShow = function()
		...
		if 	slider.check then
			slider.check:SetChecked(optionDetails.GetToggle(slider))
			if slider.check:GetChecked() == true then
				slider:Enable()
			else
				slider:Disable()
			end
	...
	slider:SetScript("OnMouseWheel", function(_, delta)
		if slider:IsEnabled() == true then
			...
			if (IsShiftKeyDown() == true) and (optionDetails.Absolute ~= true) then
				...
				step = optionDetails.step or slider:GetValueStep() or 1
			end
...
function toolbelt.ListFrame(parent, optionInfo)
	...
	listBox:SetPoint("Bottom", panel:GetParent():GetParent():GetParent():GetParent(), 0, 0)
